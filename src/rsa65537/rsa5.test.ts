...
describe("RSA1 RSA65537 verification tests", () => {
  it("should accept a simple RSA signature", () => {
    const message = Bigint6144.from(4n);
    const rsaSig = Bigint6144.from(31n);
    const modul = Bigint6144.from(33n);
  ...
  it("should accept RSA signature with hardcoded valid parameters", () => {
    ...
    const message = Bigint6144.from(13n);
    const rsaSig = Bigint6144.from(rsaSign(13n, params.d, params.n));
    const modul = Bigint6144.from(params.n);
  ...
  it("should accept RSA signature with randomly generated parameters: 1024-bits (10 iterations)", async () => {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      const message = Bigint6144.from(input);
      const signature = Bigint6144.from(rsaSign(input, params.d, params.n));
      const modulus = Bigint6144.from(params.n); // domain public key
  ...
  it("should accept RSA signature with randomly generated parameters: 6144-bits (1 iterations)", async () => {
  ...
  it("should reject RSA signature with randomly generated parameters larger than 6144 bits", async () => {
    const input = await sha256Bigint("how are you!");
    const params = generateRsaParams(6208);

    const message = Bigint6144.from(input);
    const signature = Bigint6144.from(rsaSign(input, params.d, params.n));
    const modulus = Bigint6144.from(params.n);

    expect(() => rsaVerify65537(message, signature, modulus)).toThrow();
  });

  it("should reject RSA signature with non-compliant modulus: 6144 bits", async () => {
    const input = await sha256Bigint("hello!");
    const params = generateRsaParams(6144);

    const message = Bigint6144.from(input);
    const signature = Bigint6144.from(rsaSign(input, params.d, params.n));
    const modulus = Bigint6144.from(randomPrime(6144)); // Tamper with modulus

    expect(() => rsaVerify65537(message, signature, modulus)).toThrow();
  });

  it("should reject RSA signature with non-compliant input: 6144 bits", async () => {
    const input = await sha256Bigint("hello!");
    const params = generateRsaParams(6144);

    const message = Bigint6144.from(35n); // Tamper with input
    const signature = Bigint6144.from(rsaSign(input, params.d, params.n));
    const modulus = Bigint6144.from(params.n);
  ...
  it("should reject non compliant RSA signature: false private key: 6144 bits", async () => {
    const input = await sha256Bigint("hello!");
    const params = generateRsaParams(6144);

    const message = Bigint6144.from(input);
    const signature = Bigint6144.from(rsaSign(input, params.e, params.n)); // Tamper with private key
    const modulus = Bigint6144.from(params.n);

    expect(() => rsaVerify65537(message, signature, modulus)).toThrow();
  });

  it("should reject non-compliant RSA signature: false signature modulus : 6144 bits", async () => {
    const input = await sha256Bigint("hello!");
    const params = generateRsaParams(6144);

    const message = Bigint6144.from(input);
    const signature = Bigint6144.from(rsaSign(input, params.d, 1223n)); // Tamper with signature modulus
    const modulus = Bigint6144.from(params.n);
